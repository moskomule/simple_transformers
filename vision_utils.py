...
def gen_mix_collate(num_classes: int,
                    mixup_rate: float,
                    cutmix_rate: float):
    assert mixup_rate + cutmix_rate > 0
    # https://github.com/pytorch/vision/blob/main/references/classification/train.py
    t = []
    if mixup_rate > 0:
        t.append(RandomMixup(num_classes, 1, mixup_rate))
    if cutmix_rate > 0:
        t.append(RandomCutmix(num_classes, 1, cutmix_rate))

    apply = RandomChoice(t)

    def f(batch):
        return apply(*fast_collate(batch))

    return f


#######################################################################################################################
# the following code is ported
# from https://github.com/pytorch/vision/blob/v0.11.1/references/classification/transforms.py
